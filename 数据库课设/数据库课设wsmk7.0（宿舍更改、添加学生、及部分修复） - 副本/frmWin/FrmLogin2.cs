    ...
    public partial class FrmLogin2 : Form
    {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            if (this.rdoStudent.Checked)
            {
                ...
                {
                    reader.Close();
                    sqlquery = @"select stuName from student where stuNum = '" + userid + "' and stuPass='" + txtPass.Text + "'";
                    sql = new SqlCommand(sqlquery, conn);
                    reader = sql.ExecuteReader();
                    reader.Read();
                    if (reader.HasRows == false)
                    {
                        MessageBox.Show("用户名密码出错！");
                        return;
                    }
            ...
            else 
            {
                ...
                {
                    reader.Close();
                    sqlquery = @"select teachName from teacher where teachUserName = '" + userid + "' and teachPass='" + txtPass.Text + "'";
                    sql = new SqlCommand(sqlquery, conn);
                    reader = sql.ExecuteReader();
                    reader.Read();
                    if (reader.HasRows == false)
