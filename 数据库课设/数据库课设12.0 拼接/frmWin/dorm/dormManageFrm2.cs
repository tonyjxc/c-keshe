...
namespace frmWin.dorm
{
    public partial class dormManageFrm2 : Form
    {
        ...
        {
            dgvDorm.Columns[0].Visible = false;
            ...
            string sqlquery = @"select distinct buildName,dormNum,personCount,liveflag from dorm,building where buildId=fk_buildid";
        ...
        {
            DataTable dt1 = (DataTable)dgvDorm.DataSource;
            dt1.Rows.Clear();
            dgvDorm.DataSource = dt1;
        ...
        private void btnFindStu_Click(object sender, EventArgs e)
        {
            ...
            dt1.Rows.Clear();
            dgvDorm.DataSource = dt1;
            dgvDorm.Columns[0].Visible = true;
            ...
            //%stuName%
            string sqlquery = @"select stuName,buildName,dormNum,personCount,liveflag from (dorm left join building on dorm.fk_buildid=buildId)left join student on student.fk_dormId=dormId where stuName like '%" + stuName+"%'";
            SqlDataAdapter data = new SqlDataAdapter(sqlquery, conn);
            DataSet dt = new DataSet();
            data.Fill(dt, "table1");
            DataTable datatable = dt.Tables["table1"];
        ...
        public void UpdateInfo()
        {
            ...
            SqlConnection conn = new SqlConnection("server=" + MyGlobal.ip + ";database=dormitory;UID=sa;PWD=zyh@197068;Integrated Security=False");
            conn.Open();
            string sqlquery = @"select dormId,personCount,liveflag from dorm";
            SqlDataAdapter data = new SqlDataAdapter(sqlquery, conn);
            DataSet dt = new DataSet();
            data.Fill(dt, "table1");
            DataTable datatable = dt.Tables["table1"];
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                if (value1 < 4&& value2=="不可入住")
                {
                    ...
                    string sqlquery1 = @"update dorm set liveflag='可入住' where dormId="+dormid+"";
                ...
                else if(value1 >= 4 && value2 == "可入住")
                {
                    dormid = (int)datatable.Rows[i].ItemArray[0];
                    //update dorm set flag1='可入住' where dormId=5
                    string sqlquery1 = @"update dorm set liveflag='不可入住' where dormId=" + dormid + "";
                    SqlCommand mycom1 = new SqlCommand(sqlquery1, conn);
                    mycom1.ExecuteNonQuery();
        ...
        public void Updateflag2()
        {
            ...
            SqlConnection conn = new SqlConnection("server=" + MyGlobal.ip + ";database=dormitory;UID=sa;PWD=zyh@197068;Integrated Security=False");
            conn.Open();
            string sqlquery = @"select dormId,dorm_grade,warnflag from dorm";
            SqlDataAdapter data = new SqlDataAdapter(sqlquery, conn);
            DataSet dt = new DataSet();
            data.Fill(dt, "table1");
            DataTable datatable = dt.Tables["table1"];
            ...
            for (int i = 0; i < rowCount; i++)
            {
                ...
                if (credit<18)
                {
                    ...
                    string sqlquery1 = @"update dorm set warnflag=1 where dormId=" + dormid+"";
                    SqlCommand mycom1 = new SqlCommand(sqlquery1, conn);
                    mycom1.ExecuteNonQuery();
                ...
                else
                {
                    ...
                    dormid = (int)datatable.Rows[i].ItemArray[0];
                    //update dorm set flag2=1 where dormId=1
                    string sqlquery1 = @"update dorm set warnflag=0 where dormId=" + dormid + "";
                    SqlCommand mycom1 = new SqlCommand(sqlquery1, conn);
                    mycom1.ExecuteNonQuery();
