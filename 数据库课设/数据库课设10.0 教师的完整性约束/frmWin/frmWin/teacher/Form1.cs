...
namespace frmWin.teacher
{
    public partial class Form1 : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            string myinsert = @"insert into teacher(teachId,teachUserName,teachPass,sex,teachName,hireDate) values (" + teachId + "," + teachUserName + "," + teachPass + ",'" + sex + "','" + teachName + "','" + hireDate + "' )";
            SqlCommand mycom = new SqlCommand(myinsert, conn);               //定义OleDbCommnad对象并连接数据库 
            mycom.ExecuteNonQuery();
            MessageBox.Show(teachName + "教师类型已经添加成功");
            conn.Close();
        }
