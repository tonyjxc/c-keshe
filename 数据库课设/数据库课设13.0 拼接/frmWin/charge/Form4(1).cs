...
namespace frmWin.charge
{
    public partial class Form4 : Form
    {
        ...
        public void LoadTypeInfo()
        {
            ...
            string sqlquery = @"select chargeId,chargeName,payWay,fk_chargeId,fk_stuId,unitPrice from charge,chargeType where fk_stuId='" + dormtypename + "' and charge.fk_chargeId=chargeType.typeid";
        ...
        private void btnUpdate_Click_1(object sender, EventArgs e)
        {
            ...
            string sqlquery = @"select fk_stuId from charge where fk_stuId ='" + dormtypename + "'";
            SqlCommand mycom = new SqlCommand(sqlquery, conn);
            SqlDataReader reader = mycom.ExecuteReader();
            reader.Read();
            string buildid = reader[0].ToString();

            Form8 dtuf = new Form8(dormtypename);
            dtuf.Show();
        }
    }
}