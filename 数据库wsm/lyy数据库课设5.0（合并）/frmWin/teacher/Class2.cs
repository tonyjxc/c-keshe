	...
	class dormTypeWSM
	{
		...
		public void insert(Class1 d)
		{
			...
			string mysql = string.Format(@"insert into dormType(typeName,personCount,price,sex,memeo) 
											values('{0}','{1}','{2}','{3}','{4}')",
													d.teachName, d.teachUserName, d.teachPass, d.sex, d.memeo);
		...
		public void update(Class1 d)
		{
			...
			string mysql = string.Format(@"update dormType set 
										...
										d.teachUserName, d.teachPass, d.sex, d.memeo, d.teachId);
			SqlCommand sql = new SqlCommand(mysql, conn);
			SqlDataReader reader = sql.ExecuteReader();
			reader.Close();
			conn.Close();
		...
		public void delete(Class1 d)
		{
			SqlConnection conn = new SqlConnection("server=" + MyGlobal.ip + ";database=dormitory;UID=sa;PWD=zyh@197068;Integrated Security=False");
			conn.Open();
			string mysql = string.Format("delete from dormType where typeId = {0}", d.teachId);
			SqlCommand sql = new SqlCommand(mysql, conn);
		...
		public DataTable findAll(Class1 d, PageHelper page)
		{
			SqlConnection conn = new SqlConnection("server=" + MyGlobal.ip + ";database=dormitory;UID=sa;PWD=zyh@197068;Integrated Security=False");
			conn.Open();
			string mysql = string.Format("select * from dormType where typeName like '%{0}%' order by typeId asc limit {1},{2}", d.teachName, page.getBegin(), page.pageSize);
			SqlCommand sql = new SqlCommand(mysql, conn);
			SqlDataReader reader = sql.ExecuteReader();
			DataTable dt = new DataTable();
			while (reader.Read())//决定行数
		...
		public int count(Class1 d)
		{
			SqlConnection conn = new SqlConnection("server=" + MyGlobal.ip + ";database=dormitory;UID=sa;PWD=zyh@197068;Integrated Security=False");
			conn.Open();
			string mysql = string.Format("select * from dormType where typeName like '%{0}%'", d.teachName);
			SqlCommand sql = new SqlCommand(mysql, conn);
			SqlDataReader reader = sql.ExecuteReader();
			DataTable dt = new DataTable();
			while (reader.Read())//决定行数
