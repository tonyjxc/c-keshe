...
namespace frmWin
{
    public partial class FrmDormAdd : Form
    {
        ...
        public void LoadBuild()
        {
            ...
            conn.Open();
            SqlCommand sql = new SqlCommand("select buildID,buildName from building");
            //只能一行一行的读取
        ...
        public void LoadDormType()
        {
            ...
            SqlCommand sql = new SqlCommand("select typeID,typeName+Sex AS typeName from DormType");
            SqlDataReader reader = sql.ExecuteReader();
            DataTable dt = new DataTable();
            //设置id值属性和文本属性
            this.cmbDormType.ValueMember = "typeID";
            this.cmbDormType.DisplayMember = "typeName";
            this.cmbDormType.DataSource = dt;
        ...
        public void QueryDorm()
        {
            ...
            string sqlquery = @"select
                                  ...
                                  where d.fk_typeID={0} and d.fk_typeID={1}";
            //把0和1的参数传进去，用一下两个作为传进去的参数
            //用宿舍楼和宿舍类型做条件，进行查询
            sqlquery = string.Format(sqlquery, this.cmbDormType.SelectedValue, this.cmbBuild.SelectedValue);;
